...
export default class MultiPicklist extends Component {
  constructor(props) {
    super(props);
    ...
    return (
      <div className={ picklistClassNames } aria-expanded={ this.state.opened }>
        <Button
          ...
          type='neutral'
          disabled={ disabled }
          onClick={ !disabled && this.onClick.bind(this) }
          onBlur={ !disabled && this.onBlur.bind(this) }
          onKeyDown={ !disabled && this.onKeydown.bind(this) }
        >
    ...
    return (
      this.state.opened ?
        <DropdownMenu
          ...
          onBlur={ this.onBlur }
        >
          { React.Children.map(children, this.renderMultiPicklistItem.bind(this)) }
        </DropdownMenu> :
        <div ref={ node => (this.dropdown = node) } />
    );
  }

  renderMultiPicklistItem(item) {
  ...
  render() {
    ...
    return (
      <FormElement formElementRef={ node => (this.node = node) } { ...formElemProps }>
        { this.renderMultiPicklist({ ...props, id }) }
      </FormElement>
    );
  }
}
...
MultiPicklist.isFormElement = true;

export const MultiPicklistItem = ({ label, selected, children, ...props }) => (
  <DropdownMenuItem
